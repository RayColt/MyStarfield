...
static void InitStars(RenderWindow* rw) 
{
    ...
    for (int i = 0; i < g_StarCount; ++i)
    {
        ...
        rw->stars[i].z = fz * (float)max(width, height) + 1.0f;
        rw->stars[i].speed = (float)g_StarSpeed + ud01(rw->rng) * (float)g_StarSpeed;
    }
}
...
{
    if (!rw || !rw->rt) return;
    if (!rw->brush) rw->rt->CreateSolidColorBrush(D2D1::ColorF(1.0f, 1.0f, 1.0f, 1.0f), &rw->brush);
    rw->rt->BeginDraw();

    // Direct2D clears and fill to the specified color.
    rw->rt->Clear(D2D1::ColorF(D2D1::ColorF::Black));
    // set brush color (normalize 0..255 -> 0..1)
    float baseR = GetRValue(g_CurrentStarColor) / 255.0f;
    float baseG = GetGValue(g_CurrentStarColor) / 255.0f;
    float baseB = GetBValue(g_CurrentStarColor) / 255.0f;
    rw->brush->SetColor(D2D1::ColorF(baseR, baseG, baseB, 1.0f));
    ...
    for (auto& star : rw->stars)
    {
        ...
        if (px >= 0 && px < width && py >= 0 && py < height)
        {
            ...
            rw->rt->FillRectangle(&starRect, rw->brush);
        }
    }
...
static BOOL CALLBACK MonEnumProc(HMONITOR hMon, HDC, LPRECT, LPARAM)
{
    ...
    // Create the window on this thread. Use NULL owner; rw->hwnd is not initialized yet.
    HWND hwnd = CreateWindowExW(
        WS_EX_TOPMOST,
        L"MyStarfieldFullClass",
        L"MyStarfield",
        WS_POPUP | WS_VISIBLE,
...
static void RunFull()
{
    // Register the full-screen window class used by MonEnumProc
    WNDCLASSW wcFull = {};
    wcFull.lpfnWndProc = FullWndProc;
    wcFull.hInstance = g_hInst;
    wcFull.lpszClassName = L"MyStarfieldFullClass";
    wcFull.hCursor = LoadCursor(NULL, IDC_ARROW);
    wcFull.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
    RegisterClassW(&wcFull);

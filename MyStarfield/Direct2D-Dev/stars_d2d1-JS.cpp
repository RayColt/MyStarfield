...
static void InitStars(RenderWindow* rw)
{
    ...
    for (int i = 0; i < g_StarCount; ++i)
    {
        ...
        rw->stars[i].x = fx / RAND_MAX * width - width / 2.0f;
		rw->stars[i].y = fy / RAND_MAX * height - height / 2.0f;
		rw->stars[i].z = fz / RAND_MAX * width;
		rw->stars[i].speed = g_StarSpeed + ud01(rw->rng) * g_StarSpeed; // Vary speed slightly
